...
const newsButton=document.createElement('button');newsButton.id='menu-news';newsButton.textContent='📜 Nyheter / Ändringslogg';document.querySelector('#menu-backup').after(newsButton);newsButton.onclick=()=>{rememberPage('news');document.querySelector('#guide').hidden=true;document.querySelector('#backup-guide').hidden=true;document.querySelector('#info-title').textContent='Nyheter / Ändringslogg';document.querySelector('#info-content').innerHTML='<div class="steps"><div class="step"><b>SENASTE NYTT</b><h3>📊 Statistik och 📝 anteckningar</h3><p>Se slutförandeprocent, antal utförda uppgifter samt hur många som blivit klara idag och denna vecka. Du kan också lägga till en kort anteckning till varje uppgift.</p></div><div class="step"><b>TIDIGARE</b><h3>🎯 Dagens fokus</h3><p>Välj en uppgift som dagens fokus och ändra eller ta bort den när du vill.</p></div><div class="step"><b>TIDIGARE</b><h3>💾 Säkerhetskopia</h3><p>Exportera och importera listan direkt från menyn.</p></div></div>';document.querySelector('#side-menu').classList.remove('open');document.querySelector('#info-guide').hidden=false;document.querySelector('#info-guide').scrollTo(0,0)};
